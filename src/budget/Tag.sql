...
SELECT 
    'form' AS component,
    './tagUpsert.sql' || COALESCE('?id=' || $id, '') as action,
    (SELECT CASE WHEN $id IS NOT NULL THEN 'Edit' ELSE 'Add' END || ' a tag') AS title,
...
SELECT 
    ...
    FROM BudTag
    WHERE TagId = $id::int OR $id IS NULL LIMIT 1) as value;
